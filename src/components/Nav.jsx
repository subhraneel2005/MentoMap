...
function Nav() {
  ...
  return (
    <div className='w-full px-7 py-3 bg-transparent flex justify-between top-0'>
        ...
        {user ? <img src={user?.image} alt={user?.name} className='h-10 w-10 rounded-full shadow-xl'/> : <button onClick={() => router.push('/api/auth/signin')} className='bg-gray-300 px-7 py-2 rounded-[24px] border-2 border-gray-700  text-black hover:bg-gray-400 duration-300'>Sign in</button>}
